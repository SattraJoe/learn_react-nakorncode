    ...
    <>
      <div>
        <form onSubmit={submit}>
          ...
          />
          <button className="ml-1 bg-blue-600 hover:bg-blue-700 text-white py-1 px-3 rounded cursor-pointer">
            Send
          </button>
        </form>
      </div>
  ...
  return (
    <div>
      <h1 className="font-bold mb-1">
        (Preserve) Chat online (
        {friendSelected
          ? `Send message to ${friendSelected}`
          : "No friend selected"}
        )
      </h1>
        ...
        {showMessageBox && (
          <ChatBox
            onSendMessage={(message) =>
              alert(`Send message to ${friendSelected}: ${message}`)
            }
          />
        )}
        {/* Show message box button 
        * Toggle the visibility of the message box with resetting its state.
         */}
        <button
          type="button"
        ...
        >
          {showMessageBox ? 'Hide message box' : 'Show message box'} 
        </button>
      </div>
    </div>
  );
...
function ChatWithReset() {
  ...
  return (
    <div>
      ...
      <ChatList onChangeFriend={setFriendSelected} />
      <div className="mt-2">
        {/* Insert a unique key to reset the state. */}
